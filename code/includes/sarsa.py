...
class Sarsa():
  ...
  def __init__(self, a_min, a_max, a_delta, state_size, random_chance = 0.1, learningRate = 0.1, discount = 0.9):
  ...
  def updateQ(self, action, state, targetOut):
    self.mlp.train(np.concatenate([state, action]), targetOut, 0.5)
