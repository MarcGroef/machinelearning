...
class Cacla():
  ...
  def __init__(self, a_dim, a_min, a_max, state_size, random_chance = 0.01, learningRate = 0.01, discount = 0.99):
    self.a_max = np.asarray(a_max)
    self.a_min = np.asarray(a_min)
    self.action_size = a_dim
    self.state_size = state_size
    self.action_mlp = MLP(self.state_size, 2, [10, 10], 1)
    self.value_mlp = MLP(self.state_size, 2, [10, 10], 1)
  ...
  def chooseAction(self, s):
    # generate action = best action + random sample of normal distribution
      ...
      #action = [action]
    #else:
      action = self.getExplorationAction(s, 0.01)
